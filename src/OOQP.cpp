...
MatrixXd MinJerkPoly(deque<Vec4> MJwaypoints,int xyzyaw,deque<double> ts, int n_order,double v0, double a0, double ve, double ae){
  ...
  if (ooqp>0){  //use OOQP
    ...
    for (int i = 0; i<nnzA; i++){
      ...
      jcolA[i] = vectemp(1);
      dA[i] =    vectemp(2);
      // cout << irowA[i] << " " << jcolA[i] << " " << dA[i] << endl;
    }
    /* start ooqp */
...
double poly_val(VectorXd poly, double ttt){ // For MinJerkTraj output use
  double val = 0;
  int n = poly.size();
  for (int i=0; i<n; i++){
...
VectorXd polys_vals(MatrixXd polys){ // For MinJerkTraj output use
  VectorXd vals;
  int idx = 0;
  // for (int i=0; i<tt.size(); i++){
  //   cout << "tt" << "[" << i << "]: " << tt.at(i) << endl; 
  // }
  // for (int i=0; i<ts.size(); i++){
  //   cout << "ts" << "[" << i << "]: " << ts.at(i) << endl; 
  // }
  int N = tt.size();
